"""FastAPI backend for Travel Planning Assistant.

Orchestrates multi-agent travel planning with LangGraph,
handles WebSocket real-time updates, and manages booking simulation.
"""
...
from .models import ConversationTurn, SharedState, ActiveRun, AgentType, Intent
        ...
        if assistant_messages:
            ...
            if current_intent in [Intent.FLIGHT, Intent.COMBINED]:
                ...
                if session.shared_state.flight_results:
                    await manager.send_json(session_id, {
                        "type": "flight_results",
                        "results": [f.dict() for f in session.shared_state.flight_results],
                        "session_id": session_id
                    })
            ...
            if current_intent in [Intent.HOTEL, Intent.COMBINED]:
                ...
                if session.shared_state.hotel_results:
                    await manager.send_json(session_id, {
                        "type": "hotel_results",
                        "results": [h.dict() for h in session.shared_state.hotel_results],
                        "session_id": session_id
                    })
...
async def update_user_preferences(session_id: str, pref: PreferenceUpdate):
    ...
    return {"status": "updated", "preference": pref.dict()}
